...
export class LoadToolsService {
  ...
  async agent() {
    ...
    const maxOutputTokens = parseInt(process.env.OPENAI_MAX_TOKENS || '8192', 10);
    const rawModel = aiProvider(modelName);

    const wrappedModel = new Proxy(rawModel, {
      get(target, prop) {
        const orig = (target as any)[prop];
        if ((prop === 'doGenerate' || prop === 'doStream') && typeof orig === 'function') {
          return (...args: any[]) => {
            const opts = args[0] || {};
            if (!opts.maxOutputTokens && !opts.maxTokens) {
    ...
    const agent = new Agent({
      ...
      - Never add photo credits, watermarks, or "generated by AI" mentions to post content.
      ${renderArray(
        [
...
`;
    ...
    agent.stream = (async (...args: any[]) => {
      for (let attempt = 0; attempt <= 3; attempt++) {
        try {
          return await originalStream(...args);
        } catch (err: any) {
          if (attempt < 3 && APICallError.isInstance(err) && err.isRetryable) {
            const delay = 2000 * Math.pow(2, attempt);
            console.warn(`[agent-retry] retry ${attempt + 1}/3 in ${delay}ms — status: ${err.statusCode}`);
